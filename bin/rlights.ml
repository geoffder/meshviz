...
(* TODO: think about re-writing this to make it fit in with my Lighting module a
   bit better. Also, is it worth changing lighting.fs itself? It is modiified
    from the multiple light sources raylib demo as it is. I suppose it is nice
    to have the option to quickly throw in multiple sources for now though.  *)
...
let set_position t shader p =
  Vector3.set_x t.position (OCADml.V3.x p);
  Vector3.set_y t.position (OCADml.V3.y p);
  Vector3.set_z t.position (OCADml.V3.z p);
  set_shader_value
    shader
    t.loc_pos
